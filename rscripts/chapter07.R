...
# 실습: 데이터 셋 구조 보기 
names(dataset) # 컬럼 조회
attributes(dataset) # 열 이름, 행이름, 자료구조 정보 제공 
str(dataset) # 자료 구조, 관측치와 변수의 개수 등을 제공


# 다양한 방법으로 데이터 셋 조회하기 
# 단계 1: 데이터 셋에서 특정 변수 조회하기 
dataset$age # 변수 값 출력
dataset$resident # 변수 값 출력
length(dataset$age) # 데이터 수 확인

#단계 2: 특정 변수의 조회 결과를 변수에 저장하기 
x <- dataset$gender
y <- dataset$price
...
boxplot(dataset$price) # 극단치가 너무 커서 값이 잘 안보임. 
...
setwd("C:/Rwork/Part-II")
new_data <- read.csv("new_data.csv", header = TRUE, fileEncoding = "euc-kr", encoding = "UTF-8")
str(new_data)
...
barplot(resident_gender, beside = T, horiz = T, 
        col = rainbow(5), 
        legend = row.names(resident_gender), # 범례로 행이름 사용 
...
# 단계 1: 데이터 파일 가져오기 
setwd("C:/Rwork/Part-II")
user_data <- read.csv("user_data.csv", header = T,  fileEncoding = "euc-kr", encoding = "UTF-8")
head(user_data)
...
# 단계 1: 데이터 파일 가져오기 
pay_data <- read.csv("pay_data.csv", header = T,  fileEncoding = "euc-kr", encoding = "UTF-8")
head(pay_data, 10)
